...
public class HdfsDirectorySpooler {
  ...
  public void init() {
    try {
      ...
      if (!Strings.isNullOrEmpty(firstFile)) {
        currentFile = new Path(firstFile);
      }
  ...
  private void addFileToQueue(FileStatus file) {
    ...
    if (valid || (!filesQueue.contains(file) && compare(currentFile, file.getPath()) >= 0)) {
  ...
  private int compare(Path path1, Path path2) {
  ...
  void findAndQueueFiles() throws IOException {
    ...
    FileStatus[] matchingFile = fileSystem.globStatus(new Path(spoolDirPath + "/" + pattern));
